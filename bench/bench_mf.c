 ...
 * isolates what peakfft's peak-only path is worth on this workload.
 *
...
int main(int argc,char**argv){
  ...
  pf_mf_run(mp,0,D,0,T,bs,0.f,pk,cnt,ws,we);
  ...
  printf("  %-24s %10.1f us total  %9.3f us/pair   vs MKL %.2fx  vs amd %.2fx\n",
         "peakfft (incl. ingest)",bp*1e6,bp*1e6/np,bm/bp,ba/bp);
  printf("  (sink %zu)\n",(size_t)sink);
  return 0;
}
