...
mb = dev.load_module_from_source("b4", pathlib.Path("big.slang").read_text())
kflat = dev.create_compute_kernel(dev.link_program([mb],[mb.entry_point("fusedBig")]))
kref  = dev.create_compute_kernel(dev.link_program([mb],[mb.entry_point("refineBig")]))
...
def flat(): kflat.dispatch(thread_count=[PAIRS*WG,1,1], data=bd, tmpl=bh, peakMag=bm, dump=bdump, ntmpl=NT, dumpPair=999999)
def hier():
