...
import numpy as np, sys, pathlib, time
dev = spy.Device(type=spy.DeviceType.vulkan)
src = pathlib.Path(sys.argv[1]).read_text()
ND, NT = 8, 64; PAIRS = ND*NT
print("%-8s %-12s %-9s %s" % ("n", "max rel err", "ms", "GFLOP/s"))
for N in (1024, 2048, 4096, 8192, 16384):
    WG = N//16
    mod = dev.load_module_from_source("b%d"%N, ("#define NLEN %d\n"%N)+src)
    ...
    mk = lambda n,sz,rw,dat=None: dev.create_buffer(element_count=n, struct_size=sz,
    ...
    e = (np.abs(got-ref)/np.maximum(ref,1e-30)).max()
    best=1e9
    for _ in range(7):
    ...
    print("%-8d %-12.2e %-9.4f %.0f %s" % (N, e, best*1e3, fl/best/1e9,
                                           "" if e<1e-4 else " WRONG"))
