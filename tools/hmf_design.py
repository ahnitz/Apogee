...
def emit_table(path):
    ...
    chosen = {}
    for n in TABLE_N:
        d = design(n, Ts=TABLE_SNR, FDs=TABLE_FD)
        for key, b in d.items():
            chosen[(n,) + key] = b
        print(f"  designed N={n}", flush=True)
    L = []; w = L.append
    w("/* Generated by tools/hmf_design.py -- do not edit by hand.")
    w(" *")
    w(" * Measured design constants for the hierarchical filter: which band,")
    w(" * oversampling and tap count to use, and the coarse threshold threshold that meets a")
    w(" * given false-dismissal target.  Which band, oversample and taps to use")
    w(" * is NOT here -- that comes from the measured tuning tables shipped with")
    w(" * the package (docs/hierarchical.md).  The compiled picks that used to")
    w(" * live here were removed as a second, uncheckable answer to it.")
