...
for F in $DIR/hier-*.npz; do
  out=$(OMP_NUM_THREADS=1 $PY tools/hier_bench.py --fixture $F --no-profile --reps ${REPS:-3} 2>&1)
...
printf "%d segments  gate=%s  missed %d/%d (%s)  triggered %s%%  %s ms/seg  %sx\n" \
  $nf "${MF_GATE_MARGIN:-1.00}" $miss $tru "$(echo "scale=5;$miss/$tru"|bc)" \
