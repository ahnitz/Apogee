...
pf_mf_plan *pf_mf_create(size_t n, int ndata, int ntmpl){
  ...
  { int m=0; while(((size_t)1<<m)<n) m++;
    ...
    const char *e=getenv("PEAKFFT_GMAJOR"); if(e && !atoi(e)) p->gmajor=0;
  }
...
int pf_mf_run(pf_mf_plan *p, int d0, int nd, int t0, int nt,
  ...
  (void)spec;
  int tile = 8;
  { const char *e=getenv("PEAKFFT_MFTILE"); if(e){ int v=atoi(e); if(v>0) tile=v; } }
  int total=0;
  for(int dt=0;dt<nd;dt+=tile) for(int tt=0;tt<nt;tt+=tile){
   ...
   for(int d=dt;d<dend;d++){
    ...
    for(int t=tt;t<tend;t++){
      ...
      int r = p->gmajor ? pf_binmax_prod(p->fft,Dr,Di,Hr,Hi,binsize,threshold,
                                         peaks+row*nb,&c,PF_BACKWARD,start,end)
                        : -1;
      if(r<0){
        mulspec(Dr,Di,Hr,Hi,p->pr,p->pi,n);
        r = pf_binmax_split(p->fft,p->pr,p->pi,binsize,threshold,
                            peaks+row*nb,&c,PF_BACKWARD,start,end);
      }
      if(r<0) return -1;
