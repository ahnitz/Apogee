...
void ap_fft1024_soa(float *re, float *im,
                    const __m512 (*t4r)[32], const __m512 (*t4i)[32]);
/* Same transform, but also returns the 16 per-lane maxima of |X|^2, accumulated
   from registers inside the final stage.  ap_topk uses them as a scan threshold,
   which costs it no pass of its own. */
void ap_fft1024_soa_mag(float *re, float *im,
                        const __m512 (*t4r)[32], const __m512 (*t4i)[32],
                        __m512 *vmax);

/* AoS interleaved complex <-> SoA, vectorised.  ap_deint_c conjugates as it goes,
...
typedef struct ap_cand_s { float mag2; int idx; float re, im; } ap_cand;
