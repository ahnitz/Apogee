...
# The Apple GPU back end (metal/) is a separate extension, so the CPU library
# builds and behaves identically with or without it.  macOS only; set
# MF_NO_METAL=1 to skip it.  The kernels are compiled by Metal at run time, so
# the Metal source is embedded as a string and no Xcode `metal` tool is needed.
METAL = sys.platform == "darwin" and not os.environ.get("MF_NO_METAL")
METAL_SOURCES = ["metal/mf_metal.m", "metal/_metalmodule.c"]


def embed_metal_source(outdir):
    """Write metal/kernels.metal into a header as a C string literal."""
    with open(os.path.join(HERE, "metal", "kernels.metal")) as fh:
        src = fh.read()
    os.makedirs(outdir, exist_ok=True)
    # one literal per line, concatenated: well inside every compiler's limit
    lines = "\n".join(
        '"%s\\n"' % l.replace("\\", "\\\\").replace('"', '\\"')
        for l in src.split("\n"))
    with open(os.path.join(outdir, "mf_metal_src.h"), "w") as fh:
        fh.write("/* generated by setup.py from metal/kernels.metal */\n")
        fh.write("static const char MFM_SRC[] =\n%s;\n" % lines)


class BuildExt(build_ext):
    def build_extension(self, ext):
        if ext.name == "matchedfilter._metal":
            return self.build_metal(ext)
        objects = []
    ...
    def build_metal(self, ext):
        gen = os.path.join(self.build_temp, "metal_gen")
        embed_metal_source(gen)
        objects = []
        for src in METAL_SOURCES:
            flags = BASE + (["-fobjc-arc"] if src.endswith(".m") else [])
            objects += self.compiler.compile(
                [src],
                output_dir=os.path.join(self.build_temp, "metal"),
                include_dirs=ext.include_dirs + [gen],
                extra_postargs=flags,
                debug=self.debug,
            )
        self.compiler.link_shared_object(
            objects,
            self.get_ext_fullpath(ext.name),
            extra_postargs=["-framework", "Metal", "-framework", "Foundation"],
            debug=self.debug,
        )
...
if METAL:
    EXTENSIONS.append(Extension("matchedfilter._metal", sources=[],
                                include_dirs=["metal"]))
