...
static void run(size_t N,int K,int trials,int sign){
  ...
  for(int t=0;t<trials;t++){
    ...
    if(sign==PF_FORWARD) DftiComputeForward(h,in,mkl); else DftiComputeBackward(h,in,mkl);
  ...
  printf("  N=%-8zu K=%-3d x%-3d %-8s pf_fft max rel %.3e | pf_topk set_err=%d order_err=%d max rel %.3e\n",
         N,K,trials,sign==PF_FORWARD?"fwd":"bwd",worst_fft,set_bad,order_bad,worst_topk);
...
int main(void){
  ...
  for(int d=0;d<2;d++){
    int sg = d? PF_BACKWARD : PF_FORWARD;
    run(1024,8,30,sg);
    for(int lg=12; lg<=19; lg++) run((size_t)1<<lg,8,5,sg);
    run(1048576,8,6,sg);
    run(1048576,1,6,sg);
    run(1048576,PF_MAX_K,3,sg);
  }
