...
@pytest.mark.xfail(strict=True, reason=(
    ...
    "What is still wrong is the JOINT structure, not the mean: that model "
    "over-predicts dismissal 2-10x. The fine statistic's in-band noise is "
    "the band-limited correlation at the TRUE (continuous) lag, which is "
    "correlated with the grid samples but equal to none of them; the code "
    "above takes it from one grid sample. Derive that covariance -- the "
    "profile gives it in closed form -- rather than fitting anything."))
@pytest.mark.parametrize("thr", LOOSE_GATES)
